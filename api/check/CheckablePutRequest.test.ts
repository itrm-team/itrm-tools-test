...
import { APIService, CheckablePutRequest, RequestContext, RequestAxiosCall, RequestParameterType } from "../../../dist";
...
export class TestRequest extends CheckablePutRequest {
    constructor() {
        super({
            path: "/check",
            params: [{
...
export class InvalidCheckConfigurationTestRequest extends CheckablePutRequest {
    constructor() {
        super({
            path: "/invalid-check",
            params: [{
...
describe('testing Checkable PUT Request', () => {
    ...
    test('Refuse request when API Request does not exist', async () => {
        expect.assertions(3);
        return RequestAxiosCall.put(validKeyUrl, {}).catch(err => {
    ...
    test('Checking valid', async () => {
        ...
        return RequestAxiosCall.put(validKeyUrl, {}).then(async (response: any) => {
            expect(response.message).toMatch("Success");
        });
    });
    test('Checking invalid', async () => {
        expect.assertions(4);
        return RequestAxiosCall.put(invalidKeyUrl, {}).catch(err => {
            expect(err.code).toMatch("ERR_BAD_REQUEST");
    ...
    test('Checking invalid with no unauthorization reason', async () => {
        expect.assertions(4);
        return RequestAxiosCall.put(invalidUnknownKeyUrl, {}).catch(err => {
            expect(err.code).toMatch("ERR_BAD_REQUEST");
            expect(err.response.status).toBe(401);
            expect(err.response.statusText).toMatch("Unauthorized");
